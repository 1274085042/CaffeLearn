...
//Init()完成了网络初始化和层初始化操作
template <typename Dtype>
void Net<Dtype>::Init(const NetParameter& in_param) 
{
  // Set phase from the statei.
  // 根据NetParameter对象设置处理阶段（Train\Test）
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) 
  {
    // Inherit phase from net if unset.
	  // 遍历每个层
    if (!param.layer(layer_id).has_phase())                 //每个层的阶段标记，如果在层描述中未指定阶段，就使用net的阶段
    {							
    ...
    const LayerParameter& layer_param = param.layer(layer_id);
	  //这些layer的LayerParameter都继承自NetParameter
    ...
    // After this layer is connected, set it up.
	  // Layer连接设置完毕，将调用各个layer的Setup（）函数
    ...
    const int num_param_blobs = layers_[layer_id]->blobs().size();
	  ...
	  //保证 参数配置需要的权值blob数目不大于实际对象的权值blob数目
    CHECK_LE(param_size, num_param_blobs)
        << "Too many params specified for layer " << layer_param.name();
    ParamSpec default_param_spec;

	  //每个权值层（卷积层\全连接层）都需要经过下面的过程
...
template <typename Dtype>
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {								//计算从start层到end层的前向传播
  CHECK_GE(start, 0);
...
template <typename Dtype>
void Net<Dtype>::BackwardFromTo(int start, int end) {						//从start层反向传播到end层，start>=end
  CHECK_GE(end, 0);
...
void Net<Dtype>::Backward() {
  ...
  //如果打开了调试信息开关（在prototxt中设定），则计算所有权值的data/diff的L1、L2范数，监控其变化情况，避免发散
