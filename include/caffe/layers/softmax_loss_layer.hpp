...
//这个类比单独的SoftmaxLayer+MultinomiallogisticLossLayer在梯度数值计算上更加稳定
//Test阶段，这个层被SoftmaxLayer代替
/*
输入Blob 1为预测结果，形状为N*K*1*1，K为总类别数目，N为批量数。取值范围 （-Inf，Inf）
每个类别获得的分数score，值越大说明输入图像与该类别越接近
输入Blob 2为真实标签，形状为N*1*1*1
输出Blob为计算得到的交叉熵分类损失E，形状为1*1*1*1
...
class SoftmaxWithLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  //prob_用于存储SoftmaxLayer计算的分类预测概率
  Blob<Dtype> prob_;
